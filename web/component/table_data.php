  ...
  foreach ($rows as $row) {
    ...
    $totalDays = pg_fetch_row($result);
      ...
      if ($row[team_home] == $row[team_fav]) {
        echo "<td><input class='single-checkbox' type='radio' name='selectedTeamId[$i]' value='{$row[team_home]}'><br></td>";
        echo "<td>{$home_fav[0]}</td>";
        echo "<td>{$row['spread']}</td>";
        echo "<td>{$away_fav[0]}</td>";
        echo "<td><input class='single-checkbox' type='radio' name='selectedTeamId[$i]' value='{$row[team_away]}'><br></td>";
      } else {
        echo "<td><input class='single-checkbox' type='radio' name='selectedTeamId[$i]' value='{$row[team_away]}'><br></td>";
        echo "<td>{$away_fav[0]}</td>";
        echo "<td>{$row['spread']}</td>";
